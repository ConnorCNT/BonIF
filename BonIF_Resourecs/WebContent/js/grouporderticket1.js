...
CNTLib.GroupTicket.prototype ={
		...
		onSubmit: function (e){
			...
			if(this.pagetype=="company" || this.pagetype=="ticket"){
				...
				form.find('input[name=email]').val(this.$el.find('input[name=email1]').val()+"@"+this.$el.find('input[name=email2]').val());
				form.find('input[name=payment]').val(this.$el.find('input:radio[name=payment]').filter(':checked').val());
