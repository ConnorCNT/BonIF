$(document).ready(function(){
	...
	function createSliderCounter(){
		var getSliderLn = $(".sliderWrapper li").length - 1;
		var sliderCnt = "";
	...
	// 초기정보 (사용자 수정가능)
	var sliderController = $(".slide_wrap"); //슬라이드컨트롤러
	var sliderCounterWrapper = $(".slide_paging");	//슬라이드카운터 랩퍼
	var sliderCounter = $(".slide_paging a");	// dot
	var sliderWrapper = $(".slide_area");	//롤링 래퍼
	var sliderArr = $(".slide_control > a");	//롤링 액션 화살표
	var sliderLeft = $(".btn_prev"); // 롤링 액션 왼쪽
	var sliderRight = $(".btn_next"); // 롤링 액션 왼쪽
	var sliderTarget = $(".slide_list li");	//롤링 타겟
	var sliderTargetImg = $(".slide_list li img");	//롤링 타겟이미지
	var sliderTargetLn = $(".slide_list li").length - 1;	//롤링 이미지 개수
	...
	var imgWd = 1170;	//이미지 넓이
	var imgHt = 600;	//이미지 높이
	var dotOffsetTop = 40; // 슬라이드 카운터 와의 타겟 영역과의 마진
	var dotOffsetLeft = 0; // 슬라이드 카운터사이의 여백
	var dotImgWd = 70; //슬라이드카운터 이미지 넓이
	var dotImgHt = 36; //슬라이드카운터 이미지 높이
	...
	function initCounter(){
		sliderCounterWrapper.css({
			"margin-top" : dotOffsetTop + "px",
			"width" : "100%",
			"height" : dotImgHt
		});
		sliderCounter.css({
